...
import greyImg from "../../components/assets/greyImg.png";
import yellowImg from "../../components/assets/yellowImg.png";
import { ENumericFormat } from "scichart/types/NumericFormat";
import Box from "./Box";
import styles from "./moddule.scss";
...
const prepareData = () => {
  const delta1 =
    ...
    (DATA_LENGTH - 1);
  const delta2 =
    (bloodPressureValues[DATA_LENGTH - 1] - bloodPressureValues[0]) /
    (DATA_LENGTH - 1);
  const delta3 =
    (bloodVolumeValues[DATA_LENGTH - 1] - bloodVolumeValues[0]) /
    (DATA_LENGTH - 1);
  const delta4 =
    (bloodOxygenationValues[DATA_LENGTH - 1] - bloodOxygenationValues[0]) /
    (DATA_LENGTH - 1);
  xValues.forEach((_el, index) => {
    ecgHeartRateValues[index] =
      (ecgHeartRateValues[index] - delta1 * index - 0.7) / 0.3 + 3;
    bloodPressureValues[index] =
      (bloodPressureValues[index] - delta2 * index - 0.4) / 0.3 + 1.9;
    bloodVolumeValues[index] =
      (bloodVolumeValues[index] - delta3 * index - 0.15) / 0.3 + 1.1;
    bloodOxygenationValues[index] =
      (bloodOxygenationValues[index] - delta4 * index) / 0.2;
  });
};
prepareData();
  ...
  return (
    <div>
      <div className={styles.ChartContainer}>
        <div id={divElementId} className={styles.VitalSigns} />
        <div className={styles.InfoBoxContainer}>
          <div className={styles.InfoBox} style={{ color: COLOR_GREEN }}>
            <div className={styles.IbRow1}>
              <div className={styles.IbRow1Col1}>ECG</div>
            </div>
            <div className={styles.IbRow2}>
              <div className={styles.IbRow2Col1}>
                <div>
                  V1 - 1.4MM
                  <br />
                  ST | +0.6 || +0.9
              ...
              <div className={styles.IbRow2Col2}>
                <div>{infoEcg}</div>
              </div>
            </div>
          </div>
          <div className={styles.InfoBox} style={{ color: COLOR_YELLOW }}>
            <div className={styles.IbRow1}>
              <div className={styles.IbRow1Col1}>NIBP</div>
              <div className={styles.IbRow1Col2}>
                AUTO
                <br />
                145/95
              </div>
          ...
          <div className={styles.InfoBox} style={{ color: COLOR_GREY }}>
            <div className={styles.IbRow1}>
              <div className={styles.IbRow1Col1}>SV</div>
              <div className={styles.IbRow1Col2}>
                ML 100
                <br />
                %**** 55
          ...
          <div className={styles.InfoBox} style={{ color: COLOR_BLUE }}>
            <div className={styles.IbRow1}>
              <div className={styles.IbRow1Col1}>
                SPO<span style={{ fontSize: 12 }}>2</span>
              </div>
              <div className={styles.IbRow1Col2}>18:06</div>
            </div>
            <div className={styles.IbRow2}>
              <div className={styles.IbRow2Col1}>
                <div>
              ...
              <div className={styles.IbRow2Col2}>{infoBloodOxygenation}</div>
            </div>
          </div>
        </div>
      </div>
